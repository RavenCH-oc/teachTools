...
# Phase 9 local live quiz boundary

Question Bank data is never the live authority after publish. Rust grades submissions from `SessionQuestion` snapshots only. Correct answers, accepted fill values, grading configuration, scores, and other participants' submissions are omitted from Student DTOs. Reveal delivers only the authenticated participant's own outcome and the minimal type-specific correct-answer projection.
