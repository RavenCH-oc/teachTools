...
Migration `0006_peer_review_foundation` adds six peer review tables without modifying migrations 0001–0005. Activities capture immutable Essay submission IDs at OPEN and optionally pin a SessionGroupSet revision. Claims consume capacity atomically; feedback appends immutable revisions using expectedBaseRevision and a client reviewSubmissionId accepting RFC4122 UUIDv4/v7. Other internal Peer Review IDs remain UUIDv7. Closing/ending preserves history; accepted exact replay is recognized before OPEN validation without bypassing authorization. Feedback does not change grades or Phase 10 statistics. Phase 12B–12D provide Teacher setup, authenticated Student delivery and Teacher monitoring/history. See [Peer Review](peer-review.md) for current lifecycle, ownership, bounded SQL pagination and transport boundaries. Migrations 0001–0006 are unchanged by integration closeout; no 0007 is introduced.
