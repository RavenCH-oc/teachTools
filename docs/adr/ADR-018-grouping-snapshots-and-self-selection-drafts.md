...
Teacher preset/session UI, Student self-selection/delivery, random/manual creation, and revision workflows are implemented through Phase 11E. Phase 12 Cross Group Peer Review now references the durable `sessionGroupSetId` without changing grouping lifecycle; later regrouping cannot replace its frozen membership or labels. Group statistics remain a separate concern. Cosmetic grouping polish and drag/drop remain deferred.
