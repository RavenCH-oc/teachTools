...
export function SessionAnalysisPage({ api, session, onBack, onPeerReview }: Props) {
  ...
  return <section className="session-analysis-page">
    <div className="page-heading compact"><div><p className="eyebrow">教師工作區 / 課堂統計</p><h2>課堂統計</h2><p className="intro">{session.classroomName} · {statistics.sessionState === "ACTIVE" ? "進行中" : "已結束"} · {formatSessionDate(session.endedAt ?? session.createdAt)}</p></div><button className="button ghost" type="button" onClick={onBack}>返回{session.state === "ACTIVE" ? "即時測驗" : "課堂紀錄"}</button></div>
...
function DifficultSection({ difficultQuestions }: { difficultQuestions: QuestionDifficulty[] }) {
  ...
  return <section className="analysis-section" aria-labelledby="analysis-difficult"><div className="analysis-section-heading"><div><p className="eyebrow">易錯題分析</p><h3 id="analysis-difficult">易錯題</h3></div></div>{top.length === 0 ? <p className="analysis-empty">目前沒有可計算的易錯題。</p> : <ol className="difficult-list">{top.map((item) => <li key={item.sessionQuestionId}><span>{item.position + 1}. {truncate(item.prompt)}</span><strong>{formatRate(item.accuracy)} · {item.gradedCount} 份已評分</strong></li>)}</ol>}</section>;
}
