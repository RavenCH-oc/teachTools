...
interface Props { api: LocalSessionLobbyApi; classrooms: Classroom[]; onError: (message: string) => void; onClearError: () => void; }

export function LocalSessionLobbyPage({ api, classrooms, onError, onClearError }: Props) {
  ...
  const joinUrl = useMemo(() => candidateUrl && session?.state === "LOBBY" ? `${candidateUrl}/student/join/${session.joinCode}` : "", [candidateUrl, session]);
  ...
  return <section className="local-lobby-page" aria-labelledby="local-lobby-title">
    <header className="page-heading"><div><p className="eyebrow">本機課堂</p><h2 id="local-lobby-title">建立學生等候大廳</h2><p className="intro">選擇課堂、開放大廳，再由你選擇學生要使用的區網網址。</p></div></header>
