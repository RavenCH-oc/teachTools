...
export function QuestionEditor({ api, questionSetId, question, position, onSaved, onCancel, onError }: Props) {
  ...
  useEffect(() => {
    ...
    void api.listQuestionAssets(question.id)
      ...
      .catch(() => { if (active) onError("Could not load question media."); });
    return () => { active = false; };
  }, [api, question?.id]);
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (errors.length) return onError(errors[0] ?? "Question is invalid.");
    setSaving(true);
    try {
      const saved = question
        ? await api.updateQuestion(question.id, asCreateInput(draft))
        : await api.createQuestion(asCreateInput(draft));
      onSaved(saved);
    } catch {
      onError("The Question could not be saved. Please review the fields and try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return <div className="editor-column">
    <form className="form-card question-editor" onSubmit={save}>
      <div className="editor-title-row"><div><p className="eyebrow">{question ? "Edit question" : "New question"}</p><h3>{question ? "Question details" : "Create a question"}</h3></div>{dirty && <span className="dirty-pill">Unsaved</span>}</div>
      <label className="field"><span>Question type</span><select aria-label="Question type" value={draft.type} onChange={(event) => changeType(event.target.value as QuestionType)}>{Object.entries(QUESTION_TYPE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      <label className="field"><span>Prompt</span><textarea aria-label="Question prompt" value={draft.prompt} onChange={(event) => update({ prompt: event.target.value })} maxLength={10000} placeholder="Write the question prompt" rows={4} /></label>
      <label className="field points-field"><span>Points</span><input min={1} step={1} type="number" value={draft.points} onChange={(event) => update({ points: Number(event.target.value) })} /></label>
      ...
      {draft.type === "essay" && <p className="editor-note">Essay questions have no correct-answer field. Grading remains pending for future review workflows.</p>}
      <QuestionAssetEditor api={api} assets={assets} onChange={setAssets} onError={onError} questionId={question?.id ?? null} />
      <div className="form-actions"><button className="button primary" disabled={saving} type="submit">{saving ? "Saving…" : question ? "Save changes" : "Create question"}</button><button className="button ghost" onClick={cancel} type="button">Cancel</button></div>
    </form>
