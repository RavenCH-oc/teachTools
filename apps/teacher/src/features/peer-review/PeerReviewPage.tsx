...
export function PeerReviewPage({api,sessionId,initialQuestionId,onBack,onDirtyChange,onMonitor}: {api: PeerReviewApi;sessionId:string;initialQuestionId?:string;onBack:()=>void;onDirtyChange?:(value:boolean)=>void;onMonitor?:(id:string)=>void}) {
  ...
  if(!context || !form)return <section className="state-card"><h2>同儕互評</h2><p role="alert">{error || "無法載入互評設定。"}</p><button className="button ghost" type="button" onClick={()=>void refresh()} disabled={busy}>重試</button><button className="button ghost" type="button" onClick={back}>返回即時測驗</button></section>;
  ...
  return <section className="peer-review-page">
    ...
    <form className="form-card" onSubmit={event=>{event.preventDefault();save();}}>
      ...
      {(!current || current.state==="DRAFT") && <div aria-label="開放前檢查"><h4>開放前檢查</h4>{form.mode==="CROSS_GROUP" ? <><p>可參與互評組別：{preflight?.eligible_group_count ?? 0}</p><ul>{preflight?.groups.map((g,i)=><li key={i}>{g.name} — {g.essay_count>0 ? `有 ${g.essay_count} 份論述答案` : "無可用作品"}</li>)}</ul></> : <><p>可參與互評：{question?.eligible_participant_count ?? 0} 人</p><p>可供選擇的作品：{question?.eligible_participant_count ?? 0}</p></>}{block && <p role="status">{block}</p>}<p>開放互評時會再次確認參與資格。</p></div>}
      {duplicate && canEdit && <p role="alert">此題已有草稿或進行中的活動。<button className="button ghost" type="button" onClick={()=>choose(duplicate.id)}>管理既有活動</button></p>}
      {current && current.state!=="DRAFT" && <><p>已固定作品：{current.frozen_target_count}</p><p>互評使用開放當下固定的作品版本。</p></>}
      {dirty && <p role="status">尚有未儲存的變更</p>}
      <div className="form-actions">
        {current && (current.state==="OPEN" || current.state==="CLOSED") && onMonitor && <button className="button ghost" type="button" disabled={busy} onClick={()=>onMonitor(current.id)}>查看互評進度</button>}
