...
export function LiveQuizPage({ api, onError, onOpenSessionAnalysis, onOpenSessionGrouping, onOpenPeerReview }: { api: LiveQuizApi; onError: (message: string) => void; onOpenSessionAnalysis?: (session: LocalSession) => void; onOpenSessionGrouping?: (session: LocalSession) => void; onOpenPeerReview?: (sessionId: string, questionId?: string) => void }) {
  ...
  useEffect(() => { if (!setId) { setQuestions([]); return; } void api.listQuestions(setId).then(setQuestions).catch((cause) => onError(cause instanceof Error ? cause.message : "無法載入題目。")); }, [setId]);
  ...
  return <section className="live-quiz-page"><div className="page-heading compact"><div><p className="eyebrow">教師工作區 / 即時測驗</p><h2>{session.state === "LOBBY" ? "\u6e96\u5099\u958b\u59cb\u8ab2\u5802" : "\u5373\u6642\u6e2c\u9a57"}</h2><p className="intro">{session.classroomName}</p></div><div className="form-actions">{onOpenSessionGrouping && <button className="button ghost" type="button" onClick={() => onOpenSessionGrouping(session)}>課堂分組</button>}{onOpenSessionAnalysis && session.state === "ACTIVE" && <button className="button ghost" type="button" onClick={() => onOpenSessionAnalysis(session)}>課堂統計</button>}</div></div>
    ...
    {session.state === "ACTIVE" && onOpenPeerReview && <button className="button ghost" type="button" onClick={()=>onOpenPeerReview(session.id,activeQuestion?.type === "essay" ? activeQuestion.id : undefined)}>同儕互評</button>}
